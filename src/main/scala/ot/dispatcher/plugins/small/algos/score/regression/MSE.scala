 ...
 */
case class MSE(labelCol: String, predictionCol: String, df: DataFrame, keywords: Map[String, String], searchId: Int) {
  ...
  def makeEvaluate(): DataFrame = {
    df.withColumn("error", pow(df(labelCol) - df(predictionCol), 2))
      .select("error")
      .agg(avg("error") as "MSE")
  }
}

object MSE extends ScoreModel {
  override def score(modelName: String,
    ...
    df => {
      val model = MSE(labelCol, predictionCol, df, keywords, searchId)
      model.makeEvaluate()
    }
}
