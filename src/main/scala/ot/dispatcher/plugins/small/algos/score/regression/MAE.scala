 ...
 */
case class MAE(labelCol: String, predictionCol: String, df: DataFrame, keywords: Map[String, String], searchId: Int) {

  /**
   * Performs MAE metric calculation
   *
   * @return dataframe with one column: metricName with metric value
   */
  def makeEvaluate(): DataFrame = {
    df.withColumn("error", abs(df(labelCol) - df(predictionCol)))
      .select("error")
      .agg(avg("error") as "MAE")
  }
}

object MAE extends ScoreModel {
  override def score(modelName: String,
    ...
    df => {
      val model = MAE(labelCol, predictionCol, df, keywords, searchId)
      model.makeEvaluate()
    }
}
