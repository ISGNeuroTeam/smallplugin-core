...
class LOF(override val uid: String) extends Transformer with LOFParams {
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    println("indexedPointsRDD", session.createDataFrame(indexedPointsRDD))
    ...
    println("neighborhoodRDD", session.createDataFrame(neighborhoodRDD))
